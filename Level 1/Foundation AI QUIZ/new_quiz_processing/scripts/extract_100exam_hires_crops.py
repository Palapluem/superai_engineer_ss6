...
PAGES = ROOT / "100exam_hires_pages"
CROPS = ROOT / "100exam_hires_crops"
CONTACT = ROOT / "100exam_hires_contact_sheets"
ZOOM = 4
...
def detect_labels(img: np.ndarray) -> list[tuple[int, int, int, int]]:
    ...
    for cnt in contours:
        ...
        if 60 <= w <= 190 and 50 <= h <= 120 and x < img.shape[1] * 0.35:
            boxes.append((x, y, w, h))
    boxes.sort(key=lambda b: (b[1], b[0]))
    dedup = []
    for box in boxes:
        if not dedup or abs(box[1] - dedup[-1][1]) > 80:
            dedup.append(box)
    return dedup
...
def crop_questions(paths: list[Path]) -> None:
    ...
    for page_path in paths:
        ...
        for i, (x, y, bw, bh) in enumerate(boxes):
            next_y = boxes[i + 1][1] if i + 1 < len(boxes) else h - (70 * ZOOM // 2)
            top = max(0, y - 70)
            bottom = min(h, next_y - 36)
            left = max(0, x - 130)
