#!/usr/bin/env python3
"""Report checkpoint progress for the render-only dense statement fast path."""
...
def status(args: argparse.Namespace) -> int:
    ...
    expected_rows = sum(
        sum(len(slot_ids) for slot_ids in slot_ids_by_level(schemas[artifact_id]).values())
        for artifact_id in expected_ids
    )

    completed = failed = rows = fallback_rows = 0
    elapsed_seconds = 0.0
    for artifact_id in expected_ids:
